...
- ✅ 一个 **Aurora PostgreSQL 集群**（例如集群标识符 `tb-fdr-eu`）
...
resource "aws_iam_role" "rds_proxy" {
  name = "tb-fdr-eu-rds-proxy-role"          # 与截图中的角色名一致
    ...
    Statement = [
      {
        ...
        Resource = ["arn:aws:secretsmanager:eu-west-1:959491356696:secret:fxps/fdr/rds/sa/secret-rBe6OT"]  # 替换为您的 Secret ARN
      }
    ]
  })
}
...
resource "aws_db_proxy" "main" {
  name                   = "tb-fdr-eu-rds-proxy"          # 与截图一致
  ...
  auth {
    ...
    secret_arn  = "arn:aws:secretsmanager:eu-west-1:959491356696:secret:fxps/fdr/rds/sa/secret-rBe6OT"  # 替换
...
data "aws_rds_cluster" "aurora" {
  cluster_identifier = "tb-fdr-eu"            # 替换为您的集群标识符
}
...
- **读写端点**：`tb-fdr-eu-rds-proxy.proxy-xxxxxx.eu-west-1.rds.amazonaws.com`
- **只读端点**：`tb-fdr-eu-rds-proxy.read-only.endpoint.proxy-xxxxxx.eu-west-1.rds.amazonaws.com`
...
const client = new Client({
  host: 'tb-fdr-eu-rds-proxy.proxy-xxxxxx.eu-west-1.rds.amazonaws.com',
  port: 5432,
...
const readOnlyClient = new Client({
  host: 'tb-fdr-eu-rds-proxy.read-only.endpoint.proxy-xxxxxx.eu-west-1.rds.amazonaws.com',
  port: 5432,
  user: 'db_admin',
  password: 'your_password',
  database: 'fxdb',
...
4. **验证状态**  
   ...
   - 您应该看到 `tb-fdr-eu-rds-proxy` 状态为 **Available**
...
- **原因**：Proxy 尚未完全就绪时就尝试添加目标。Terraform 通常会等待，但有时也会出现依赖缺失。
